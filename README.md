...
1. **Использование класса Student из Lab08**
   - Переиспользование методов `to_dict()` и `from_dict()`
   - Автоматическая валидация через `__post_init__`

2. **Использование утилит из Lab04**
   - Функция `write_csv()` из `src/lib/io_helpers.py`
   - Используется в методах `__init__()`, `remove()` и `update()`
   - Обеспечивает единообразную работу с CSV

3. **Работа с CSV через стандартную библиотеку**
   - `csv.DictReader` для чтения с автоматическим парсингом заголовков
   - `csv.writer` для добавления записей
   - Параметр `newline=""` для корректной работы на всех ОС

4. **Обработка ошибок**
   - Проверка существования файла через `try/except FileNotFoundError`
   - Валидация полей через `set(HEADER).issubset(r.keys())`
   - Создание родительских директорий через `mkdir(parents=True, exist_ok=True)`

5. **Атомарность операций UPDATE и DELETE**
   - Полное чтение файла в память
   - Модификация данных
   - Полная перезапись файла через `write_csv()` (предотвращает повреждение при сбое)

6. **Case-insensitive поиск**
   - Метод `find()` игнорирует регистр через `lower()`
   - Удобен для поиска по неточным совпадениям